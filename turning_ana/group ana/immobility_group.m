...
for i = 1:length(file)
    ...
    %delay is defined by the time when the change of speed is larger than 4
    %times of SD of the baseline.
    temp2 = find(abs(data.backward_mean(:,pre_stim/bin+1:(pre_stim+xP2)/bin)-mean(data.backward_mean(:,1:pre_stim/bin))) > 2,1,'first')
%     temp2 = find(abs(data.backward_mean(:,pre_stim/bin+1:(pre_stim+xP2)/bin)) <1,1,'first');
    if isempty(temp2)
        temp2 = 60
    end
...
angular_v = 1;
speed_ylim = [0 20]; %%%angular_v   define by yourself
